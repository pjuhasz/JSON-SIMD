...
eval { JSON::XS->new->use_simdjson(1)->allow_nonref (1)->decode ('01') }; ok $@ =~ /leading zero/;
eval { JSON::XS->new->use_simdjson(1)->allow_nonref->decode ('00') }; ok $@ =~ /leading zero/;
eval { JSON::XS->new->use_simdjson(1)->allow_nonref (1)->decode ('-0.') }; ok $@ =~ /decimal point/;
eval { JSON::XS->new->use_simdjson(1)->allow_nonref->decode ('-0e') }; ok $@ =~ /exp sign/;
eval { JSON::XS->new->use_simdjson(1)->allow_nonref (1)->decode ('-e+1') }; ok $@ =~ /initial minus/;
