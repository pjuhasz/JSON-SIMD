...
splitter +JSON::XS->new->use_simdjson(1)->allow_nonref (0), '  ["x\\"","\\u1000\\\\n\\nx",1,{"\\\\" :5 , "": "x"}]';
splitter +JSON::XS->new->use_simdjson(1)->allow_nonref (0), '[ "x\\"","\\u1000\\\\n\\nx" , 1,{"\\\\ " :5 , "": " x"} ] ';
splitter +JSON::XS->new->use_simdjson(1)                  , '"test"';
splitter +JSON::XS->new->use_simdjson(1)                  , ' "5" ';
splitter +JSON::XS->new->use_simdjson(1)                  , '-1e5';
splitter +JSON::XS->new->use_simdjson(1)                  , ' 0.00E+00 ';

{
   my $text = '[5],{"":1} , [ 1,2, 3], {"3":null}';
   my $coder = JSON::XS->new->use_simdjson(1);
   for (0 .. length $text) {
      my $a = substr $text, 0, $_;
      my $b = substr $text, $_;
...
{
   my $coder = JSON::XS->new->use_simdjson(1)->max_size (5);
   ok (!$coder->incr_parse ("[    "), "incsize1");
   eval { !$coder->incr_parse ("]  ") }; ok ($@ =~ /6 bytes/, "incsize2 $@");
}

{
   my $coder = JSON::XS->new->use_simdjson(1)->max_depth (3);
